...
def fetch_open(filename: str, local_prefix: str = "", emoji: bool = False):
    ...
    fetches it from `https://www.unicode.org/Public/`. Retries on errors."""
    basename = os.path.basename(filename)
    if local_prefix and not os.path.exists(local_prefix):
        os.makedirs(local_prefix, exist_ok=True)
    localname = os.path.join(
        local_prefix, basename) if local_prefix else basename

    if not os.path.exists(localname):
        prefix = f"emoji/{UNICODE_VERSION[:-2]}" if emoji else f"{UNICODE_VERSION}/ucd"
        url = f"https://www.unicode.org/Public/{prefix}/{filename}"
...
""")
        ...
        for i, table in enumerate(tables):
            ...
            if table.bytes_per_row is None:
                ...
                if table.cfged:
                    module.write("// CJK only\n")
                module.write(f"let {table.name.lower()} : ReadOnlyArray[UInt] = [")
...
let {table.name.lower()} : ReadOnlyArray[ReadOnlyArray[UInt]] = [\n"""
                )
...
let non_transparent_zero_widths : ReadOnlyArray[(Int, Int)] = [
"""
        )
...
///| FIXME: can we get better compression?
// CJK only
let solidus_transparent : ReadOnlyArray[(Int, Int)] = [
"""
        )
...
let emoji_presentation_leaves : ReadOnlyArray[ReadOnlyArray[UInt]] = [
"""
        )
        ...
        for leaf_idx, leaf in enumerate(text_presentation_leaves):
            module.write(
                f"""
let text_presentation_leaf_{leaf_idx} : ReadOnlyArray[(UInt, UInt)] = [
"""
            )
            for lo, hi in leaf:
        ...
        for leaf_idx, leaf in enumerate(emoji_modifier_leaves):
            module.write(
                f"""
let emoji_modifier_leaf_{leaf_idx} : ReadOnlyArray[(UInt, UInt)] = [
"""
            )
            for lo, hi in leaf:
                module.write(f"    (0x{lo:02X}, 0x{hi:02X}),\n")
